...
collect_args <- function(){
  ...
  parser$add_argument('--vec_strain_names', type = 'character', help = 'Strain names')
...
strain_names <- read.table(args$vec_strain_names) %>% as_tibble() %>% select(V1) %>% unlist() %>% as.character()

complete_df_identical_sequences <- function(df_identical_sequences_pairsnp, strain_names){
  df_symmetric <- df_identical_sequences_pairsnp %>% 
    rename(strain_A = strain_1) %>% rename(strain_1 = strain_2) %>% rename(strain_2 = strain_A) %>% 
    select(strain_1, strain_2, n_mutations) %>% 
    bind_rows(df_identical_sequences_pairsnp)
  
  df_symmetric %>% 
    bind_rows(tibble(strain_1 = strain_names, strain_2 = strain_names, n_mutations = rep(0, length(strain_names)))) %>% 
    return()
}

# Get cluster allocation from pairs of identical sequences
get_cluster_alloc_from_df_identical_sequences <- function(df_identical_sequences_pairsnp, strain_names){
  df_identical_sequences <- complete_df_identical_sequences(df_identical_sequences_pairsnp, strain_names)
...
if(nrow(df_identical_sequences) > 0){
  df_cluster_alloc <- get_cluster_alloc_from_df_identical_sequences(df_identical_sequences, strain_names) %>%
